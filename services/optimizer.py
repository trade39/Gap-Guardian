...
Includes optimization for entry window times and passes data_interval_str.
"""
import pandas as pd
import numpy as np
import itertools
...
def _run_single_backtest_for_optimization(
    ...
    risk_per_trade_percent: float, data_interval_str: str # Added data_interval_str
) -> dict:
    ...
    trades_df, equity_s, perf_metrics = backtester.run_backtest(
        ...
        data_interval_str=data_interval_str # Pass to backtester
    )
...
def run_grid_search(
    ...
    param_value_lists: dict, data_interval_str: str, # Added data_interval_str
    ...
    for i, combo_values in enumerate(value_combinations):
        ...
        params_for_backtest = {
            'SL Points': current_params_dict['sl_points'], 'RRR': current_params_dict['rrr'],
        ...
        try:
            perf = _run_single_backtest_for_optimization(
                params_for_backtest, price_data, initial_capital, risk_per_trade_percent, data_interval_str
            )
...
def run_random_search(
    price_data: pd.DataFrame, initial_capital: float, risk_per_trade_percent: float,
    param_ranges: dict, num_iterations: int, data_interval_str: str, # Added data_interval_str
    progress_callback=None
) -> pd.DataFrame:
    ...
    for i in range(num_iterations):
        ...
        for p_name, p_config in param_ranges.items():
            if isinstance(p_config, list): current_params_dict[p_name] = random.choice(p_config)
            else:
                p_min, p_max = p_config; val = random.uniform(p_min, p_max)
...
def run_walk_forward_optimization(
    ...
    wfo_params: dict, opt_algo: str, opt_params_config: dict, data_interval_str: str, # Added data_interval_str
    progress_callback=None
    ...
    while True:
        ...
        if opt_algo == "Grid Search":
            grid_param_values = {
                'sl_points': opt_params_config['sl_values'], 'rrr': opt_params_config['rrr_values'],
                ...
                'entry_end_minute': opt_params_config.get('entry_end_minute_values', [settings.DEFAULT_ENTRY_WINDOW_END_MINUTE])
            }
            optimization_results_df = run_grid_search(in_sample_data, initial_capital, risk_per_trade_percent, grid_param_values, data_interval_str)
        elif opt_algo == "Random Search":
            random_param_ranges = {
                'sl_points': opt_params_config['sl_range'], 'rrr': opt_params_config['rrr_range'],
            ...
            optimization_results_df = run_random_search(in_sample_data, initial_capital, risk_per_trade_percent, random_param_ranges, opt_params_config['iterations'], data_interval_str)
    ...
    if not final_oos_trades_df.empty:
        ...
        temp_equity_values = [initial_capital]; temp_equity_dates = [full_price_data.index.min() if not full_price_data.empty else pd.Timestamp.now(tz='UTC') - pd.Timedelta(days=1)] # Fallback for empty price data
        if not final_oos_trades_df_sorted.empty and not final_oos_trades_df_sorted['EntryTime'].dropna().empty:
             temp_equity_dates = [final_oos_trades_df_sorted['EntryTime'].dropna().min() - pd.Timedelta(seconds=1)] # Start equity right before first OOS trade
        else: # No OOS trades, or no valid entry times
             temp_equity_dates = [full_price_data.index.min() if not full_price_data.empty else pd.Timestamp.now(tz='UTC') - pd.Timedelta(days=1)]
        ...
        if len(temp_equity_dates) > 0:
            ...
            if not chained_oos_equity.empty:
                # Create a date range covering all OOS periods for reindexing
                min_oos_date = min(d['OutOfSampleStart'] for d in wfo_log if d.get('OutOfSampleStart')) if wfo_log and any(d.get('OutOfSampleStart') for d in wfo_log) else chained_oos_equity.index.min()
                max_oos_date = max(d['OutOfSampleEnd'] for d in wfo_log if d.get('OutOfSampleEnd')) if wfo_log and any(d.get('OutOfSampleEnd') for d in wfo_log) else chained_oos_equity.index.max()
                
                if pd.NaT not in [min_oos_date, max_oos_date] and min_oos_date <= max_oos_date : # Ensure valid date range
                    oos_full_date_range = pd.date_range(start=min_oos_date, end=max_oos_date, freq='B') # Business days
                    if not oos_full_date_range.empty:
                        chained_oos_equity = chained_oos_equity.reindex(chained_oos_equity.index.union(oos_full_date_range)).ffill().loc[oos_full_date_range.min():oos_full_date_range.max()]

        else: chained_oos_equity = pd.Series([initial_capital], index=[full_price_data.index.min() if not full_price_data.empty else pd.Timestamp.now(tz='UTC') - pd.Timedelta(days=1)])
