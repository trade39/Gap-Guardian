...
selected_ticker_name = st.sidebar.selectbox("Select Symbol:", options=list(settings.DEFAULT_TICKERS.keys()), index=0, key="ticker_sel_v3")
...
selected_timeframe_display = st.sidebar.selectbox(
    ...
    key="timeframe_selector_ui_main_v3",
...
current_interval = st.session_state.selected_timeframe # This is the data_interval_str
...
start_date = st.sidebar.date_input("Start Date:", value=default_start_date_value, min_value=min_allowable_start_date_for_ui, max_value=max_possible_start_date, key=f"start_date_{current_interval}_v3", help=f"Start date. {date_input_help_suffix}")
...
end_date = st.sidebar.date_input("End Date:", value=default_end_date_value, min_value=min_end_date_value, max_value=today, key=f"end_date_{current_interval}_v3", help=f"End date. {date_input_help_suffix}")
...
sl_points_single = st.sidebar.number_input("SL (points):", 0.1, value=settings.DEFAULT_STOP_LOSS_POINTS, step=0.1, format="%.2f", key="sl_s_man")
rrr_single = st.sidebar.number_input("RRR:", 0.1, value=settings.DEFAULT_RRR, step=0.1, format="%.1f", key="rrr_s_man")
st.sidebar.markdown("**Entry Window (NY Time - Manual Run):**")
c1,c2=st.sidebar.columns(2); entry_start_hour_single = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man")
entry_start_minute_single = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man")
c1,c2=st.sidebar.columns(2); entry_end_hour_single = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man")
entry_end_minute_single = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man", help="Usually 00.")

analysis_mode = st.sidebar.radio("Analysis Type:", ("Single Backtest", "Parameter Optimization", "Walk-Forward Optimization"), 0, key="analysis_mode_v3")

opt_algo = settings.DEFAULT_OPTIMIZATION_ALGORITHM
sl_min, sl_max, sl_steps = settings.DEFAULT_SL_POINTS_OPTIMIZATION_RANGE.values()
rrr_min, rrr_max, rrr_steps = settings.DEFAULT_RRR_OPTIMIZATION_RANGE.values()
esh_min, esh_max, esh_steps = settings.DEFAULT_ENTRY_START_HOUR_OPTIMIZATION_RANGE.values()
esm_vals = settings.DEFAULT_ENTRY_START_MINUTE_OPTIMIZATION_VALUES
eeh_min, eeh_max, eeh_steps = settings.DEFAULT_ENTRY_END_HOUR_OPTIMIZATION_RANGE.values()
eem_vals = settings.DEFAULT_ENTRY_END_MINUTE_OPTIMIZATION_VALUES
rand_iters = settings.DEFAULT_RANDOM_SEARCH_ITERATIONS
opt_metric = settings.DEFAULT_OPTIMIZATION_METRIC

if analysis_mode != "Single Backtest":
    st.sidebar.markdown("##### In-Sample Optimization Settings")
    opt_algo = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo), key="opt_algo_v3")
    opt_metric = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric), key="opt_metric_v3")
    st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min=c1.number_input("Min",value=sl_min,step=0.1,format="%.1f",key="slmin_o3")
    sl_max=c2.number_input("Max",value=sl_max,step=0.1,format="%.1f",key="slmax_o3")
    if opt_algo=="Grid Search": sl_steps=c3.number_input("Steps",2,10,int(sl_steps),1,key="slsteps_o3")
    st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min=c1.number_input("Min",value=rrr_min,step=0.1,format="%.1f",key="rrrmin_o3")
    rrr_max=c2.number_input("Max",value=rrr_max,step=0.1,format="%.1f",key="rrrmax_o3")
    if opt_algo=="Grid Search": rrr_steps=c3.number_input("Steps",2,10,int(rrr_steps),1,key="rrrsteps_o3")
    st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min=c1.number_input("Min Hr",value=esh_min,min_value=0,max_value=23,step=1,key="eshmin_o3")
    esh_max=c2.number_input("Max Hr",value=esh_max,min_value=0,max_value=23,step=1,key="eshmax_o3")
    if opt_algo=="Grid Search": esh_steps=c3.number_input("Hr Steps",2,5,int(esh_steps),1,key="eshsteps_o3")
    esm_vals=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals, key="esmvals_o3")
    if not esm_vals: esm_vals = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
    st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min=c1.number_input("Min Hr",value=eeh_min,min_value=0,max_value=23,step=1,key="eehmin_o3")
    eeh_max=c2.number_input("Max Hr",value=eeh_max,min_value=0,max_value=23,step=1,key="eehmax_o3")
    if opt_algo=="Grid Search": eeh_steps=c3.number_input("Hr Steps",2,5,int(eeh_steps),1,key="eehsteps_o3")
    # eem_vals (Entry End Minute) kept fixed from settings for simplicity
    if opt_algo=="Random Search": rand_iters=st.sidebar.number_input("Random Iterations:",10,500,rand_iters,10,key="randiter_o3")
    if opt_algo=="Grid Search": st.sidebar.caption(f"Grid Combs: {int(sl_steps*rrr_steps*esh_steps*len(esm_vals)*eeh_steps)}")
    else: st.sidebar.caption(f"Random Iterations: {rand_iters}")

wfo_isd,wfo_oosd,wfo_sd = settings.DEFAULT_WFO_IN_SAMPLE_DAYS, settings.DEFAULT_WFO_OUT_OF_SAMPLE_DAYS, settings.DEFAULT_WFO_STEP_DAYS
if analysis_mode == "Walk-Forward Optimization":
    st.sidebar.markdown("##### WFO Settings (Days)"); wfo_isd=st.sidebar.number_input("In-Sample:",30,value=wfo_isd,step=10,key="wfoisd_v3")
    wfo_oosd=st.sidebar.number_input("Out-of-Sample:",10,value=wfo_oosd,step=5,key="wfoosd_v3")
    wfo_sd=st.sidebar.number_input("Step:",min_value=wfo_oosd,value=wfo_sd,step=5,key="wfosd_v3")
...
if st.sidebar.button("Run Analysis", type="primary", use_container_width=True, key="run_main_v3"):
    ...
    if start_date >= end_date: st.error(f"Error: Start date ({start_date}) must be before end date ({end_date}).")
        ...
        if price_data_df.empty: st.warning(f"No price data for {selected_ticker_name} ({current_interval}). Cannot proceed.")
            ...
            manual_entry_start_t = dt_time(entry_start_hour_single, entry_start_minute_single)
            manual_entry_end_t = dt_time(entry_end_hour_single, entry_end_minute_single)

            if analysis_mode == "Single Backtest":
                st.subheader("Single Backtest Run")
                with st.spinner("Running..."):
                    signals = strategy_engine.generate_signals(price_data_df.copy(), sl_points_single, rrr_single, manual_entry_start_t, manual_entry_end_t)
                    st.session_state.signals = signals
                    trades, equity, perf = backtester.run_backtest(price_data_df.copy(), signals, initial_capital, risk_per_trade_percent, sl_points_single, current_interval)
                    st.session_state.backtest_results = {"trades":trades,"equity_curve":equity,"performance":perf,"params":{"SL":sl_points_single,"RRR":rrr_single,"TF":current_interval,"Entry":f"{manual_entry_start_t:%H:%M}-{manual_entry_end_t:%H:%M}","src":"Manual"}}
                ...
                param_cfg = {'metric_to_optimize': opt_metric,
                             'entry_end_minute_values': settings.DEFAULT_ENTRY_END_MINUTE_OPTIMIZATION_VALUES} # Fixed end minute
                if opt_algo == "Grid Search":
                    param_cfg.update({'sl_values':np.linspace(sl_min,sl_max,int(sl_steps)), 'rrr_values':np.linspace(rrr_min,rrr_max,int(rrr_steps)),
                                      'entry_start_hour_values':[int(h) for h in np.linspace(esh_min,esh_max,int(esh_steps))],
                                      'entry_start_minute_values':esm_vals,
                                      'entry_end_hour_values':[int(h) for h in np.linspace(eeh_min,eeh_max,int(eeh_steps))]})
                else: # Random Search
                    param_cfg.update({'sl_range':(sl_min,sl_max), 'rrr_range':(rrr_min,rrr_max), 'iterations':rand_iters,
                                      'entry_start_hour_range':(esh_min,esh_max), 'entry_start_minute_values':esm_vals,
                                      'entry_end_hour_range':(eeh_min,eeh_max)})
                ...
                if analysis_mode == "Parameter Optimization":
                    ...
                    with st.spinner(f"Running {opt_algo}..."):
                        if opt_algo == "Grid Search": opt_df = optimizer.run_grid_search(price_data_df, initial_capital, risk_per_trade_percent, param_cfg, current_interval, lambda p,s: opt_cb(p,s))
                        else: opt_df = optimizer.run_random_search(price_data_df, initial_capital, risk_per_trade_percent, param_cfg, rand_iters, current_interval, lambda p,s: opt_cb(p,s))
                        ...
                        if not opt_df.empty:
                            ...
                            valid_opt = opt_df.dropna(subset=[opt_metric])
                            if not valid_opt.empty:
                                best_r = valid_opt.loc[valid_opt[opt_metric].idxmin()] if opt_metric=="Max Drawdown (%)" else valid_opt.loc[valid_opt[opt_metric].idxmax()]
                                st.session_state.best_params_from_opt = {"SL":best_r["SL Points"],"RRR":best_r["RRR"],"ES_H":best_r["EntryStartHour"],"ES_M":best_r["EntryStartMinute"],"EE_H":best_r["EntryEndHour"],"EE_M":best_r.get("EntryEndMinute",0),"Metric":best_r[opt_metric]}
                                ...
                                st.info(f"Best for '{opt_metric}': SL={best_r['SL Points']:.2f}, RRR={best_r['RRR']:.1f}, Entry={best_es_t:%H:%M}-{best_ee_t:%H:%M} (Val: {best_r[opt_metric]:.2f})")
                                with st.spinner("Running backtest with best parameters..."):
                                    ...
                                    st.session_state.backtest_results = {"trades":trades_b,"equity_curve":equity_b,"performance":perf_b,"params":{"SL":best_r["SL Points"],"RRR":best_r["RRR"],"TF":current_interval,"Entry":f"{best_es_t:%H:%M}-{best_ee_t:%H:%M}","src":f"Opt ({opt_algo})"}}
                            else: st.warning(f"No valid results for '{opt_metric}' in optimization.")
                        else: st.error("Optimization yielded no results.")
                elif analysis_mode == "Walk-Forward Optimization":
                    st.subheader(f"Walk-Forward Optimization ({opt_algo})")
                    wfo_p = {'in_sample_days':wfo_isd,'out_of_sample_days':wfo_oosd,'step_days':wfo_sd}
                    with st.spinner(f"Running WFO with {opt_algo}... This will take considerable time."):
                        wfo_log,oos_trades,oos_equity,oos_perf = optimizer.run_walk_forward_optimization(price_data_df,initial_capital,risk_per_trade_percent,wfo_p,opt_algo,param_cfg,current_interval,lambda p,s: opt_cb(p,s))
...
# --- Display Area ---
# ... (Display logic from previous version, ensuring it handles new param info in titles/summaries and tab recreation) ...
        ...
        if st.session_state.backtest_results:
            ...
            with detail_tabs[3+idx_offset]:
                if not st.session_state.price_data.empty:
                    ...
                    csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v3')
    ...
    if opt_tab_idx != -1 and len(active_tabs) > opt_tab_idx:
        with active_tabs[opt_tab_idx]:
            ...
            if not opt_df.empty:
                ...
                csv_opt = opt_df.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt, f"{ticker_symbol}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v3')
                st.markdown("#### Optimization Heatmap (SL vs RRR - Full Period)")
                opt_metric_hm = opt_metric if analysis_mode == "Parameter Optimization" else settings.DEFAULT_OPTIMIZATION_METRIC
                if opt_algo == "Grid Search" and 'SL Points' in opt_df.columns and 'RRR' in opt_df.columns:
                    heatmap_fig = plotting.plot_optimization_heatmap(opt_df, 'SL Points', 'RRR', opt_metric_hm)
                    st.plotly_chart(heatmap_fig, use_container_width=True)
                else: st.info("Heatmap for SL vs RRR is generated for Grid Search when these are primary optimized parameters. For other parameters or Random Search, review the table.")
    ...
    if wfo_tab_idx != -1 and len(active_tabs) > wfo_tab_idx:
        with active_tabs[wfo_tab_idx]:
            if st.session_state.wfo_results:
                ...
                csv_wfo_log = wfo_res["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log, f"{ticker_symbol}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v3')
                ...
                if not wfo_res["oos_trades"].empty:
                    ...
                    csv_wfo_trades = wfo_res["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades, f"{ticker_symbol}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v3')
                else: st.info("No out-of-sample trades generated during WFO.")
            else: st.info("No WFO results. Run 'Walk-Forward Optimization' mode.")

elif st.sidebar.button("Clear Results", use_container_width=True, key="clear_button_main_page_tf_v3"):
...
st.sidebar.markdown("---")
st.sidebar.info(f"App Version: 0.4.0 | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}") # Version increment
