...
selected_ticker_name = st.sidebar.selectbox("Select Symbol:", options=list(settings.DEFAULT_TICKERS.keys()), index=0, key="ticker_sel_v12")
...
selected_timeframe_display = st.sidebar.selectbox("Select Timeframe:", options=list(settings.AVAILABLE_TIMEFRAMES.keys()), index=default_tf_display_index, key="timeframe_selector_ui_main_v12")
...
default_start_date_value = (today - timedelta(days=min(settings.MAX_SHORT_INTRADAY_DAYS - 5 if settings.MAX_SHORT_INTRADAY_DAYS else 15, max_history_limit_days -1 if max_history_limit_days else 15))) if ui_current_interval in settings.YFINANCE_SHORT_INTRADAY_INTERVALS + settings.YFINANCE_HOURLY_INTERVALS else (today - timedelta(days=30*7 if ui_current_interval=="1wk" else 365)) # Adjusted default for longer TFs
...
start_date_ui = st.sidebar.date_input("Start Date:", value=default_start_date_value, min_value=min_allowable_start_date_for_ui, max_value=max_possible_start_date, key=f"start_date_widget_{ui_current_interval}_v12", help=f"Start date. {date_input_help_suffix}")
...
end_date_ui = st.sidebar.date_input("End Date:", value=default_end_date_value_ui, min_value=min_end_date_value_ui, max_value=today, key=f"end_date_widget_{ui_current_interval}_v12", help=f"End date. {date_input_help_suffix}")
...
sl_points_single_ui = st.sidebar.number_input("SL (points):", 0.1, value=settings.DEFAULT_STOP_LOSS_POINTS, step=0.1, format="%.2f", key="sl_s_man_v12")
rrr_single_ui = st.sidebar.number_input("RRR:", 0.1, value=settings.DEFAULT_RRR, step=0.1, format="%.1f", key="rrr_s_man_v12")
...
if selected_strategy_name == "Gap Guardian":
    ...
    c1,c2=st.sidebar.columns(2); entry_start_hour_single_ui = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man_v12")
    entry_start_minute_single_ui = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man_v12")
    c1,c2=st.sidebar.columns(2); entry_end_hour_single_ui = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man_v12")
    entry_end_minute_single_ui = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man_v12", help="Usually 00.")
...
analysis_mode_ui = st.sidebar.radio("Analysis Type:", ("Single Backtest", "Parameter Optimization", "Walk-Forward Optimization"), 0, key="analysis_mode_v12")
...
if analysis_mode_ui != "Single Backtest":
    ...
    opt_algo_ui = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo_ui), key="opt_algo_v12")
    opt_metric_ui = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric_ui), key="opt_metric_v12")
    
    st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min_opt_ui=c1.number_input("Min",value=sl_min_opt_ui,step=0.1,format="%.1f",key="slmin_o12")
    sl_max_opt_ui=c2.number_input("Max",value=sl_max_opt_ui,step=0.1,format="%.1f",key="slmax_o12")
    if opt_algo_ui=="Grid Search": sl_steps_opt_ui=c3.number_input("Steps",2,10,int(sl_steps_opt_ui),1,key="slsteps_o12")
    
    st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min_opt_ui=c1.number_input("Min",value=rrr_min_opt_ui,step=0.1,format="%.1f",key="rrrmin_o12")
    rrr_max_opt_ui=c2.number_input("Max",value=rrr_max_opt_ui,step=0.1,format="%.1f",key="rrrmax_o12")
    if opt_algo_ui=="Grid Search": rrr_steps_opt_ui=c3.number_input("Steps",2,10,int(rrr_steps_opt_ui),1,key="rrrsteps_o12")

    if selected_strategy_name == "Gap Guardian":
        st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min_opt_ui=c1.number_input("Min Hr",value=esh_min_opt_ui,min_value=0,max_value=23,step=1,key="eshmin_o12")
        esh_max_opt_ui=c2.number_input("Max Hr",value=esh_max_opt_ui,min_value=0,max_value=23,step=1,key="eshmax_o12")
        if opt_algo_ui=="Grid Search": esh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(esh_steps_opt_ui),1,key="eshsteps_o12")
        esm_vals_opt_ui=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals_opt_ui, key="esmvals_o12")
        if not esm_vals_opt_ui: esm_vals_opt_ui = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
        st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min_opt_ui=c1.number_input("Min Hr",value=eeh_min_opt_ui,min_value=0,max_value=23,step=1,key="eehmin_o12")
        eeh_max_opt_ui=c2.number_input("Max Hr",value=eeh_max_opt_ui,min_value=0,max_value=23,step=1,key="eehmax_o12")
        if opt_algo_ui=="Grid Search": eeh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(eeh_steps_opt_ui),1,key="eehsteps_o12")
    
    if opt_algo_ui=="Random Search": rand_iters_ui=st.sidebar.number_input("Random Iterations:",10,500,rand_iters_ui,10,key="randiter_o12")
...
if analysis_mode_ui == "Walk-Forward Optimization":
    ...
    MIN_WFO_IS_DAYS, MIN_WFO_OOS_DAYS = 30, 10
    ...
    if total_available_days_for_wfo >= MIN_WFO_IS_DAYS + MIN_WFO_OOS_DAYS:
        ...
        calculated_isd = max(MIN_WFO_IS_DAYS, calculated_isd); calculated_oosd = max(MIN_WFO_OOS_DAYS, calculated_oosd); calculated_stepd = max(calculated_oosd, calculated_stepd) # Ensure step >= OOS
    ...
    else: st.sidebar.caption(f"Total period ({total_available_days_for_wfo}d) is short for WFO.")
    ...
    wfo_sd_ui = st.sidebar.number_input("Step (Days):", min_value=wfo_oosd_ui, value=st.session_state.wfo_sd_ui_val, step=5, key="wfosd_v12") # Step must be >= OOS
...
strategy_info_md = f"Selected Strategy: **{selected_strategy_name}** | Timeframe: **{selected_timeframe_display}** ({st.session_state.selected_timeframe_value})"
if selected_strategy_name == "Gap Guardian":
        ...
        if price_data_df.empty: st.warning(f"No price data for {selected_ticker_name} ({interval_for_this_run}). Cannot proceed.")
            ...
            if analysis_mode_ui == "Single Backtest":
                ...
                with st.spinner("Running..."):
                    ...
                    if selected_strategy_name == "Gap Guardian": param_display += f", Entry: {strategy_params_for_engine['entry_start_time']:%H:%M}-{strategy_params_for_engine['entry_end_time']:%H:%M}"
                ...
                if analysis_mode_ui == "Parameter Optimization":
                    ...
                    with st.spinner(f"Running {opt_algo_ui}..."):
                        if opt_algo_ui == "Grid Search": opt_df = optimizer.run_grid_search(price_data_df, initial_capital_ui, risk_per_trade_percent_ui, actual_params_to_optimize_config, interval_for_this_run, optimizer_control_params, opt_cb)
                        else: opt_df = optimizer.run_random_search(price_data_df, initial_capital_ui, risk_per_trade_percent_ui, actual_params_to_optimize_config, interval_for_this_run, optimizer_control_params, opt_cb)
                        ...
                        if not opt_df.empty:
                            ...
                            if not valid_opt.empty:
                                ...
                                best_params_for_bt_opt = {'strategy_name': selected_strategy_name, 'stop_loss_points': best_r["SL Points"], 'rrr': best_r["RRR"]}
                                ...
                                if selected_strategy_name == "Gap Guardian":
                                    ...
                                    best_params_for_bt_opt['entry_start_time'] = best_es_t; best_params_for_bt_opt['entry_end_time'] = best_ee_t
                                ...
                                with st.spinner("Running backtest with best parameters..."):
                                    signals_b = strategy_engine.generate_signals(data=price_data_df.copy(), **best_params_for_bt_opt)
                    ...
                    wfo_opt_config_and_control = {**actual_params_to_optimize_config, **optimizer_control_params}
                    with st.spinner(f"Running WFO with {opt_algo_ui}... This will take considerable time."):
                        wfo_log,oos_trades,oos_equity,oos_perf = optimizer.run_walk_forward_optimization(price_data_df,initial_capital_ui,risk_per_trade_percent_ui,wfo_p, opt_algo_ui, wfo_opt_config_and_control,interval_for_this_run,opt_cb)
...
if main_tabs_to_display_names:
    tabs_key_string = "_".join(main_tabs_to_display_names) + f"_{st.session_state.run_analysis_clicked_count}_{selected_strategy_name}"
    ...
    if "📊 Backtest Performance" in tab_map:
        with tab_map["📊 Backtest Performance"]:
            if st.session_state.backtest_results:
                ...
                entry_display_val = run_params.get("EntryDisplay", "")
                if entry_display_val:
                    if "SL:" in entry_display_val and "RRR:" in entry_display_val : param_info += f" | {entry_display_val}"
                    else: param_info += f" | SL: {run_params.get('SL', 'N/A'):.2f}, RRR: {run_params.get('RRR', 'N/A'):.1f}, Entry: {entry_display_val}"
                elif run_params.get("SL") is not None and run_params.get("RRR") is not None: param_info += f" | SL: {float(run_params.get('SL')):.2f}, RRR: {float(run_params.get('RRR')):.1f}"
                param_info += ")"
                st.markdown(f"#### Performance Summary{param_info}")
                POSITIVE_COLOR, NEGATIVE_COLOR, NEUTRAL_COLOR = settings.POSITIVE_METRIC_COLOR, settings.NEGATIVE_METRIC_COLOR, settings.NEUTRAL_METRIC_COLOR # Use theme-appropriate neutral color
                ...
                def display_styled_metric(col, lbl, val, raw, c=True, pct=False, p=2, pf_logic=False, mdd_logic=False):
                    fmt_val = format_metric_display(val, p, c, pct); clr = NEUTRAL_COLOR # Will be overridden by theme's text color
                    # Color logic based on value for specific metrics
                    if not (pd.isna(raw) or raw is None):
                        if pf_logic:
                            if raw > 1: clr = POSITIVE_COLOR
                            elif raw < 1 and raw != 0 : clr = NEGATIVE_COLOR
                            elif raw == 0 and performance.get('Gross Profit', 0) == 0 and performance.get('Gross Loss', 0) == 0: clr = NEUTRAL_COLOR # No trades, no P/L
                            elif raw == 0 : clr = NEGATIVE_COLOR # PF of 0 usually means losses wiped out gains or only losses
                        elif mdd_logic:
                ...
                with detail_tabs[3+idx_offset]:
                    if not st.session_state.price_data.empty:
                        ...
                        st.dataframe(st.session_state.price_data.head(100), height=300, use_container_width=True) # Show more rows
                        csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v12')
    ...
    if opt_tab_idx != -1:
        with tab_map["⚙️ Optimization Results (Full Period)"]:
            ...
            if not opt_df_display.empty:
                ...
                st.dataframe(opt_df_display.style.format({col: '{:.2f}' for col in float_cols_opt_disp}), height=400, use_container_width=True) # Increased height
                csv_opt_disp = opt_df_display.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt_disp, f"{ticker_symbol}_{selected_strategy_name}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v12')
                ...
                else: st.info("Heatmap for SL vs RRR is generated for Grid Search if these parameters are optimized. Review the table for other parameters or Random Search.")
    ...
    if wfo_tab_idx != -1:
        with tab_map["🚶 Walk-Forward Analysis"]:
            if st.session_state.wfo_results:
                ...
                st.markdown(f"#### Walk-Forward Optimization Log ({selected_strategy_name})"); st.dataframe(wfo_res_disp["log"].style.format({col: '{:.2f}' for col in wfo_res_disp["log"].select_dtypes(include='float').columns if col in wfo_res_disp["log"]}), height=300, use_container_width=True)
                csv_wfo_log_disp = wfo_res_disp["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v12')
                st.markdown("#### Aggregated Out-of-Sample Trades")
                if not wfo_res_disp["oos_trades"].empty:
                    st.dataframe(wfo_res_disp["oos_trades"].style.format({col: '{:.2f}' for col in wfo_res_disp["oos_trades"].select_dtypes(include='float').columns}), height=300, use_container_width=True)
                    csv_wfo_trades_disp = wfo_res_disp["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v12')
...
st.sidebar.markdown("---")
st.sidebar.info(f"App Version: 0.6.0 | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}") # Version increment
