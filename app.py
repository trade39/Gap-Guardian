...
selected_ticker_name = st.sidebar.selectbox("Select Symbol:", options=list(settings.DEFAULT_TICKERS.keys()), index=0, key="ticker_sel_v7")
...
selected_timeframe_display = st.sidebar.selectbox("Select Timeframe:", options=list(settings.AVAILABLE_TIMEFRAMES.keys()), index=default_tf_display_index, key="timeframe_selector_ui_main_v7")
st.session_state.selected_timeframe_value = settings.AVAILABLE_TIMEFRAMES[selected_timeframe_display]
# `ui_current_interval` reflects the live selection in the UI for date calculations
...
start_date_ui = st.sidebar.date_input("Start Date:", value=default_start_date_value, min_value=min_allowable_start_date_for_ui, max_value=max_possible_start_date, key=f"start_date_widget_{ui_current_interval}", help=f"Start date. {date_input_help_suffix}")
...
end_date_ui = st.sidebar.date_input("End Date:", value=default_end_date_value_ui, min_value=min_end_date_value_ui, max_value=today, key=f"end_date_widget_{ui_current_interval}", help=f"End date. {date_input_help_suffix}")
...
sl_points_single_ui = st.sidebar.number_input("SL (points):", 0.1, value=settings.DEFAULT_STOP_LOSS_POINTS, step=0.1, format="%.2f", key="sl_s_man_v7")
rrr_single_ui = st.sidebar.number_input("RRR:", 0.1, value=settings.DEFAULT_RRR, step=0.1, format="%.1f", key="rrr_s_man_v7")
st.sidebar.markdown("**Entry Window (NY Time - Manual Run):**")
c1,c2=st.sidebar.columns(2); entry_start_hour_single_ui = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man_v7")
entry_start_minute_single_ui = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man_v7")
c1,c2=st.sidebar.columns(2); entry_end_hour_single_ui = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man_v7")
entry_end_minute_single_ui = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man_v7", help="Usually 00.")

analysis_mode_ui = st.sidebar.radio("Analysis Type:", ("Single Backtest", "Parameter Optimization", "Walk-Forward Optimization"), 0, key="analysis_mode_v7")
...
if analysis_mode_ui != "Single Backtest":
    ...
    opt_algo_ui = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo_ui), key="opt_algo_v7")
    opt_metric_ui = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric_ui), key="opt_metric_v7")
    st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min_opt_ui=c1.number_input("Min",value=sl_min_opt_ui,step=0.1,format="%.1f",key="slmin_o7")
    sl_max_opt_ui=c2.number_input("Max",value=sl_max_opt_ui,step=0.1,format="%.1f",key="slmax_o7")
    if opt_algo_ui=="Grid Search": sl_steps_opt_ui=c3.number_input("Steps",2,10,int(sl_steps_opt_ui),1,key="slsteps_o7")
    st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min_opt_ui=c1.number_input("Min",value=rrr_min_opt_ui,step=0.1,format="%.1f",key="rrrmin_o7")
    rrr_max_opt_ui=c2.number_input("Max",value=rrr_max_opt_ui,step=0.1,format="%.1f",key="rrrmax_o7")
    if opt_algo_ui=="Grid Search": rrr_steps_opt_ui=c3.number_input("Steps",2,10,int(rrr_steps_opt_ui),1,key="rrrsteps_o7")
    st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min_opt_ui=c1.number_input("Min Hr",value=esh_min_opt_ui,min_value=0,max_value=23,step=1,key="eshmin_o7")
    esh_max_opt_ui=c2.number_input("Max Hr",value=esh_max_opt_ui,min_value=0,max_value=23,step=1,key="eshmax_o7")
    if opt_algo_ui=="Grid Search": esh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(esh_steps_opt_ui),1,key="eshsteps_o7")
    esm_vals_opt_ui=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals_opt_ui, key="esmvals_o7")
    if not esm_vals_opt_ui: esm_vals_opt_ui = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
    st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min_opt_ui=c1.number_input("Min Hr",value=eeh_min_opt_ui,min_value=0,max_value=23,step=1,key="eehmin_o7")
    eeh_max_opt_ui=c2.number_input("Max Hr",value=eeh_max_opt_ui,min_value=0,max_value=23,step=1,key="eehmax_o7")
    if opt_algo_ui=="Grid Search": eeh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(eeh_steps_opt_ui),1,key="eehsteps_o7")
    if opt_algo_ui=="Random Search": rand_iters_ui=st.sidebar.number_input("Random Iterations:",10,500,rand_iters_ui,10,key="randiter_o7")
...
if analysis_mode_ui == "Walk-Forward Optimization":
    st.sidebar.markdown("##### WFO Settings (Days)"); wfo_isd_ui=st.sidebar.number_input("In-Sample:",30,value=wfo_isd_ui,step=10,key="wfoisd_v7")
    wfo_oosd_ui=st.sidebar.number_input("Out-of-Sample:",10,value=wfo_oosd_ui,step=5,key="wfoosd_v7")
    wfo_sd_ui=st.sidebar.number_input("Step:",min_value=wfo_oosd_ui,value=wfo_sd_ui,step=5,key="wfosd_v7")
...
if st.sidebar.button("Run Analysis", type="primary", use_container_width=True, key="run_main_v7"):
    ...
    # Use the interval from session state, which reflects the UI selection at the time of button click
    ...
    if start_date_ui >= end_date_ui: st.error(f"Error: Start date ({start_date_ui}) must be before end date ({end_date_ui}).")
        ...
        if price_data_df.empty: st.warning(f"No price data for {selected_ticker_name} ({interval_for_this_run}). Cannot proceed.")
            ...
            if analysis_mode_ui == "Single Backtest":
                ...
                # This dictionary contains only the strategy parameters to be optimized
                ...
                if opt_algo_ui == "Grid Search":
                    ...
                    actual_params_to_optimize_config.update({
                        'sl_points': (sl_min_opt_ui, sl_max_opt_ui), 'rrr': (rrr_min_opt_ui, rrr_max_opt_ui),
                        'entry_start_hour': (esh_min_opt_ui, esh_max_opt_ui), 'entry_start_minute': esm_vals_opt_ui,
                        'entry_end_hour': (eeh_min_opt_ui, eeh_max_opt_ui),
                        'entry_end_minute': settings.DEFAULT_ENTRY_END_MINUTE_OPTIMIZATION_VALUES
                    })
                
                # This dictionary contains control parameters for the optimization process
                ...
                if analysis_mode_ui == "Parameter Optimization":
                    ...
                    with st.spinner(f"Running {opt_algo_ui}..."):
                        ...
                        st.session_state.optimization_results_df = opt_df; prog_bar_container.progress(1.0, text="Optimization Complete!")
                        if not opt_df.empty:
                            st.success("Full period optimization finished!") # ... (Best param selection and backtest as before)
                    ...
                    # For WFO, optimizer_control_config needs both control params and strategy param definitions
                    ...
                    with st.spinner(f"Running WFO with {opt_algo_ui}... This will take considerable time."):
                        ...
                        prog_bar_container.progress(1.0, text="WFO Complete!"); st.success("Walk-Forward Optimization finished!")
            ...
            prog_bar_container.empty() # Clear progress bar after completion or error


# --- Display Area ---
# This section is now responsible for building the tab list based on available results
# and then rendering the content within those tabs.
...
if main_tabs_to_display_names:
    # Use a key that depends on the exact set of tabs being created to help Streamlit manage them
    # This is a simple way to force recreation if the tab structure changes.
    # The run_analysis_clicked_count ensures it's new on each run.
    tabs_key_string = "_".join(main_tabs_to_display_names) + f"_{st.session_state.run_analysis_clicked_count}"
    
    # Create tabs. The objects are returned in a list.
    # No need to store these tab objects in session_state if we always recreate them based on current results.
    created_tabs = st.tabs(main_tabs_to_display_names) # Removed key for now, as dynamic list with key can be tricky.

    # Map tab names to their objects for easier access
    ...
    if "📊 Backtest Performance" in tab_map:
        with tab_map["📊 Backtest Performance"]:
            # ... (Existing detailed display logic for backtest performance, as in previous version)
            if st.session_state.backtest_results:
                ...
                with detail_tabs[3+idx_offset]:
                    if not st.session_state.price_data.empty:
                        ...
                        csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v7')
    ...
    if "⚙️ Optimization Results (Full Period)" in tab_map:
        with tab_map["⚙️ Optimization Results (Full Period)"]:
            # ... (Existing display logic for optimization results) ...
            ...
            if not opt_df_display.empty:
                ...
                csv_opt_disp = opt_df_display.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt_disp, f"{ticker_symbol}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v7')
    ...
    if "🚶 Walk-Forward Analysis" in tab_map:
        with tab_map["🚶 Walk-Forward Analysis"]:
            # ... (Existing display logic for WFO results) ...
            if st.session_state.wfo_results:
                ...
                csv_wfo_log_disp = wfo_res_disp["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log_disp, f"{ticker_symbol}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v7')
                ...
                if not wfo_res_disp["oos_trades"].empty:
                    ...
                    csv_wfo_trades_disp = wfo_res_disp["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades_disp, f"{ticker_symbol}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v7')
