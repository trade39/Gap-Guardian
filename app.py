...
selected_ticker_name = st.sidebar.selectbox("Select Symbol:", options=list(settings.DEFAULT_TICKERS.keys()), index=0, key="ticker_sel_v4") # Incremented key
...
selected_timeframe_display = st.sidebar.selectbox(
    ...
    key="timeframe_selector_ui_main_v4", # Incremented key
...
start_date = st.sidebar.date_input("Start Date:", value=default_start_date_value, min_value=min_allowable_start_date_for_ui, max_value=max_possible_start_date, key=f"start_date_{current_interval}_v4", help=f"Start date. {date_input_help_suffix}") # Incremented key
...
end_date = st.sidebar.date_input("End Date:", value=default_end_date_value, min_value=min_end_date_value, max_value=today, key=f"end_date_{current_interval}_v4", help=f"End date. {date_input_help_suffix}") # Incremented key
...
sl_points_single = st.sidebar.number_input("SL (points):", 0.1, value=settings.DEFAULT_STOP_LOSS_POINTS, step=0.1, format="%.2f", key="sl_s_man_v4") # Incremented key
rrr_single = st.sidebar.number_input("RRR:", 0.1, value=settings.DEFAULT_RRR, step=0.1, format="%.1f", key="rrr_s_man_v4") # Incremented key
st.sidebar.markdown("**Entry Window (NY Time - Manual Run):**")
c1,c2=st.sidebar.columns(2); entry_start_hour_single = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man_v4") # Incremented key
entry_start_minute_single = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man_v4") # Incremented key
c1,c2=st.sidebar.columns(2); entry_end_hour_single = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man_v4") # Incremented key
entry_end_minute_single = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man_v4", help="Usually 00.") # Incremented key

analysis_mode = st.sidebar.radio("Analysis Type:", ("Single Backtest", "Parameter Optimization", "Walk-Forward Optimization"), 0, key="analysis_mode_v4") # Incremented key
...
esm_vals_opt_ui = settings.DEFAULT_ENTRY_START_MINUTE_OPTIMIZATION_VALUES
eeh_min_opt_ui, eeh_max_opt_ui, eeh_steps_opt_ui = settings.DEFAULT_ENTRY_END_HOUR_OPTIMIZATION_RANGE.values()
# eem_vals_opt_ui = settings.DEFAULT_ENTRY_END_MINUTE_OPTIMIZATION_VALUES # Kept fixed for now
...
if analysis_mode != "Single Backtest":
    ...
    opt_algo = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo), key="opt_algo_v4") # Incremented key
    opt_metric_ui = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric_ui), key="opt_metric_v4") # Incremented key
    st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min_opt_ui=c1.number_input("Min",value=sl_min_opt_ui,step=0.1,format="%.1f",key="slmin_o4") # Incr.
    sl_max_opt_ui=c2.number_input("Max",value=sl_max_opt_ui,step=0.1,format="%.1f",key="slmax_o4") # Incr.
    if opt_algo=="Grid Search": sl_steps_opt_ui=c3.number_input("Steps",2,10,int(sl_steps_opt_ui),1,key="slsteps_o4") # Incr.
    st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min_opt_ui=c1.number_input("Min",value=rrr_min_opt_ui,step=0.1,format="%.1f",key="rrrmin_o4") # Incr.
    rrr_max_opt_ui=c2.number_input("Max",value=rrr_max_opt_ui,step=0.1,format="%.1f",key="rrrmax_o4") # Incr.
    if opt_algo=="Grid Search": rrr_steps_opt_ui=c3.number_input("Steps",2,10,int(rrr_steps_opt_ui),1,key="rrrsteps_o4") # Incr.
    st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min_opt_ui=c1.number_input("Min Hr",value=esh_min_opt_ui,min_value=0,max_value=23,step=1,key="eshmin_o4") # Incr.
    esh_max_opt_ui=c2.number_input("Max Hr",value=esh_max_opt_ui,min_value=0,max_value=23,step=1,key="eshmax_o4") # Incr.
    if opt_algo=="Grid Search": esh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(esh_steps_opt_ui),1,key="eshsteps_o4") # Incr.
    esm_vals_opt_ui=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals_opt_ui, key="esmvals_o4") # Incr.
    if not esm_vals_opt_ui: esm_vals_opt_ui = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
    st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min_opt_ui=c1.number_input("Min Hr",value=eeh_min_opt_ui,min_value=0,max_value=23,step=1,key="eehmin_o4") # Incr.
    eeh_max_opt_ui=c2.number_input("Max Hr",value=eeh_max_opt_ui,min_value=0,max_value=23,step=1,key="eehmax_o4") # Incr.
    if opt_algo=="Grid Search": eeh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(eeh_steps_opt_ui),1,key="eehsteps_o4") # Incr.
    if opt_algo=="Random Search": rand_iters_ui=st.sidebar.number_input("Random Iterations:",10,500,rand_iters_ui,10,key="randiter_o4") # Incr.
    if opt_algo=="Grid Search": st.sidebar.caption(f"Grid Combs: {int(sl_steps_opt_ui*rrr_steps_opt_ui*esh_steps_opt_ui*len(esm_vals_opt_ui)*eeh_steps_opt_ui)}")
...
if analysis_mode == "Walk-Forward Optimization":
    st.sidebar.markdown("##### WFO Settings (Days)"); wfo_isd_ui=st.sidebar.number_input("In-Sample:",30,value=wfo_isd_ui,step=10,key="wfoisd_v4") # Incr.
    wfo_oosd_ui=st.sidebar.number_input("Out-of-Sample:",10,value=wfo_oosd_ui,step=5,key="wfoosd_v4") # Incr.
    wfo_sd_ui=st.sidebar.number_input("Step:",min_value=wfo_oosd_ui,value=wfo_sd_ui,step=5,key="wfosd_v4") # Incr.
...
if st.sidebar.button("Run Analysis", type="primary", use_container_width=True, key="run_main_v4"): # Incremented key
    ...
    if start_date >= end_date: st.error(f"Error: Start date ({start_date}) must be before end date ({end_date}).")
        ...
        if price_data_df.empty: st.warning(f"No price data for {selected_ticker_name} ({current_interval}). Cannot proceed.")
            ...
            if analysis_mode == "Single Backtest":
                ...
                # CORRECTED: Use base parameter names as keys for param_config_for_opt
                param_config_for_opt = {'metric_to_optimize': opt_metric_ui,
                                        'entry_end_minute_values': settings.DEFAULT_ENTRY_END_MINUTE_OPTIMIZATION_VALUES}
                if opt_algo == "Grid Search":
                    param_config_for_opt.update({
                        'sl_points': np.linspace(sl_min_opt_ui, sl_max_opt_ui, int(sl_steps_opt_ui)),
                        'rrr': np.linspace(rrr_min_opt_ui, rrr_max_opt_ui, int(rrr_steps_opt_ui)),
                        'entry_start_hour': [int(h) for h in np.linspace(esh_min_opt_ui, esh_max_opt_ui, int(esh_steps_opt_ui))],
                        'entry_start_minute': esm_vals_opt_ui,
                        'entry_end_hour': [int(h) for h in np.linspace(eeh_min_opt_ui, eeh_max_opt_ui, int(eeh_steps_opt_ui))]
                    })
                else: # Random Search
                    param_config_for_opt.update({
                        'sl_points': (sl_min_opt_ui, sl_max_opt_ui), # Range tuple
                        'rrr': (rrr_min_opt_ui, rrr_max_opt_ui),       # Range tuple
                        'iterations': rand_iters_ui,
                        'entry_start_hour': (esh_min_opt_ui, esh_max_opt_ui), # Range tuple
                        'entry_start_minute': esm_vals_opt_ui, # List of discrete values to sample from
                        'entry_end_hour': (eeh_min_opt_ui, eeh_max_opt_ui)    # Range tuple
                    })
                ...
                if analysis_mode == "Parameter Optimization":
                    ...
                    with st.spinner(f"Running {opt_algo}..."):
                        if opt_algo == "Grid Search": opt_df = optimizer.run_grid_search(price_data_df, initial_capital, risk_per_trade_percent, param_config_for_opt, current_interval, lambda p,s: opt_cb(p,s))
                        else: opt_df = optimizer.run_random_search(price_data_df, initial_capital, risk_per_trade_percent, param_config_for_opt, rand_iters_ui, current_interval, lambda p,s: opt_cb(p,s))
                    ...
                    with st.spinner(f"Running WFO with {opt_algo}... This will take considerable time."):
                        wfo_log,oos_trades,oos_equity,oos_perf = optimizer.run_walk_forward_optimization(price_data_df,initial_capital,risk_per_trade_percent,wfo_p,opt_algo,param_config_for_opt,current_interval,lambda p,s: opt_cb(p,s))
...
if st.session_state.backtest_results or not st.session_state.optimization_results_df.empty or st.session_state.wfo_results:
    ...
    with active_tabs[0]: # Backtest Performance
        if st.session_state.backtest_results:
            ...
            # Check for specific param keys before formatting
            sl_val = run_params.get("SL", run_params.get("SL Points")) 
            rrr_val = run_params.get("RRR")
            entry_val = run_params.get("Entry")

            if sl_val is not None: param_info += f" | SL: {sl_val:.2f}"
            if rrr_val is not None: param_info += f" | RRR: {rrr_val:.1f}"
            ...
            with detail_tabs[3+idx_offset]:
                if not st.session_state.price_data.empty:
                    ...
                    csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v4') # Incr.
    ...
    if opt_tab_idx != -1 and len(active_tabs) > opt_tab_idx:
        with active_tabs[opt_tab_idx]:
            ...
            if not opt_df.empty:
                ...
                csv_opt = opt_df.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt, f"{ticker_symbol}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v4') # Incr.
    ...
    if wfo_tab_idx != -1 and len(active_tabs) > wfo_tab_idx:
        with active_tabs[wfo_tab_idx]:
            if st.session_state.wfo_results:
                ...
                csv_wfo_log = wfo_res["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log, f"{ticker_symbol}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v4') # Incr.
                ...
                if not wfo_res["oos_trades"].empty:
                    ...
                    csv_wfo_trades = wfo_res["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades, f"{ticker_symbol}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v4') # Incr.
                else: st.info("No out-of-sample trades generated during WFO.")
            else: st.info("No WFO results. Run 'Walk-Forward Optimization' mode.")

elif st.sidebar.button("Clear Results", use_container_width=True, key="clear_button_main_page_tf_v4"): # Incr.
...
st.sidebar.markdown("---")
st.sidebar.info(f"App Version: 0.4.1 | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}") # Version increment
